...
- GitHub release orchestration waits for Go and Rust, preserving both existing releases.
  It does not claim a completed TypeScript publication.

A manual release of a historical tag that predates both the marker and helper keeps
TypeScript disabled and selects the Go/Rust release workflows present in that tag.
Partial state and malformed state fail closed.

Once enabled, tag pushes permit the protected publish job and GitHub orchestration
waits for `go,rust,typescript`. Manual `workflow_dispatch` is **always dry-run**, even
...
1. Confirm the npm `37signals` organization controls/approves `@37signals/hey`, its
   public name, license and first version. The repository version aligns with the
   current Go and Rust SDK release; this does not assert that npm version exists.
...
`make bump VERSION=x.y.z` updates Go, Rust and TypeScript versions, their lockfiles,
and release documentation. `make sync-api-version`
