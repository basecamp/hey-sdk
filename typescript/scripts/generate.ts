  ...
  for (const [path, item] of Object.entries(spec.paths)) {
    for (const [method, op] of Object.entries(item)) {
      ...
      if (
        responseMedia.some(
          (mediaType) =>
            mediaType !== "application/json" && mediaType !== "text/html",
        )
      )
        throw new Error(`Unsupported response media for ${id}: ${responseMedia}`);
      if (responseMedia.includes("text/html") && responseMedia.length !== 1)
        throw new Error(`Ambiguous HTML response media for ${id}: ${responseMedia}`);
      metadata[id] = {
