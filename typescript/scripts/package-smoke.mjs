...
import {generatePKCE, HEY_OAUTH_CONFIG} from '@37signals/hey/oauth';
assert.equal(VERSION, ${JSON.stringify(manifest.version)});
assert.ok(API_VERSION); assert.equal(generatePKCE().verifier.length,43);
assert.equal(HEY_OAUTH_CONFIG.token_endpoint,'https://app.hey.com/oauth/tokens');
...
import {authorizationURL, generatePKCE, HEY_OAUTH_CONFIG} from '@37signals/hey/oauth';
...
authorizationURL(HEY_OAUTH_CONFIG,{clientId:'app',redirectUri:'http://localhost/callback',state:'state',challenge:'challenge',installId:'installation'});
