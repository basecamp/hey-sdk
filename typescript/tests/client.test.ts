...
describe("HEY transport", () => {
  ...
  it("requests and returns modeled HTML without inventing a JSON representation", async () => {
    ...
    const result = await new HeyClient({ token: "secret", fetch: m.fetch })
      .getWorkflowStage(
        { path: { workflowId: 8801, stageId: 5512 } },
        { format: "json" },
      );
    expect(new URL(m.requests[0]!.url).pathname).toBe(
      "/workflows/8801/stages/5512",
    );
    expect(m.requests[0]!.headers.get("Accept")).toBe("text/html");
    expectTypeOf(result.data).toEqualTypeOf<string | undefined>();
    expect(result.data).toBe(html);
  });
  it("decodes JSON errors from modeled HTML operations", async () => {
    const m = mock([
      json({ errors: ["Stage is unavailable"] }, 422, {
        "X-Request-Id": "req-html-error",
      }),
    ]);
    await expect(
      new HeyClient({ token: "secret", fetch: m.fetch }).getWorkflowStage({
        path: { workflowId: 8801, stageId: 5512 },
      }),
    ).rejects.toMatchObject({
      code: "validation",
      message: "Stage is unavailable",
      requestId: "req-html-error",
    });
  });
  ...
  it("rejects malformed successful JSON with response metadata, does not echo HTML errors", async () => {
    const m = mock([
      new Response("<html>secret</html>", {
        headers: { "X-Request-Id": "req-garbled" },
      }),
  ...
  it.each([
    { retryAfter: "0", delayMs: 1000 },
    { retryAfter: "2", delayMs: 2000 },
  ])(
    "keeps the retry-policy floor and honors a longer Retry-After of $retryAfter seconds",
    async ({ retryAfter, delayMs }) => {
      vi.useFakeTimers();
      try {
        const m = mock([
          json({}, 503, { "Retry-After": retryAfter }),
          json([]),
        ]);
        const request = new HeyClient({ token: "secret", fetch: m.fetch })
          .listContacts({ query: { page: "cursor+with/slash" } });
  ...
  it.each([
    ...
    "preserves structured errors when Retry-After %s exceeds the timer range",
    async (retryAfter) => {
      const m = mock([
        json({ error: "quota exhausted" }, 429, {
  ...
  it("honors maxRetries=0 for a huge Retry-After", async () => {
    const m = mock([
      json({ error: "quota exhausted" }, 429, {
  ...
  it("normalizes generated Fetch and response-stream failures when retries are disabled", async () => {
    const failures = [
      vi.fn(async () => {
    ...
    for (const fetch of failures) {
      const error = await new HeyClient({
        token: "secret",
        fetch,
        maxRetries: 0,
      })
  ...
  it("retries safe operations after a transport failure", async () => {
    vi.useFakeTimers();
    try {
      const fetch = vi
      ...
      const request = new HeyClient({ token: "secret", fetch }).listBoxes();
      await vi.advanceTimersByTimeAsync(0);
      expect(fetch).toHaveBeenCalledTimes(1);
  ...
  it("coalesces concurrent refreshes and propagates failures", async () => {
    ...
    expect(refresh).toHaveBeenCalledTimes(1);
    const refreshFailure = new Error("provider failed");
    const failedRefresh = vi.fn(async () => {
      throw refreshFailure;
    });
    const m = mock([json({}, 401)]);
    const error = await new HeyClient({
      token: { getToken: () => "old", refresh: failedRefresh },
      fetch: m.fetch,
    })
      .listBoxes()
      .catch((value: unknown) => value);
    expect(error).toMatchObject({
      code: "auth_required",
      message: "HEY credentials could not be refreshed",
  ...
  it("aborts before sending or while waiting for a retry", async () => {
    ...
    await new Promise((r) => setImmediate(r));
    const reason = new Error("retry cancelled");
...
describe("HEY pagination", () => {
  ...
  it.each(["operation", "pages"])(
    "normalizes a malformed next link from %s into HeyError",
    async (entryPoint) => {
      const m = mock([
        json([], 200, { Link: "<https://[>; rel=next" }),
      ]);
      const client = new HeyClient({ token: "secret", fetch: m.fetch });
      const request =
        entryPoint === "operation"
          ? client.listContacts()
          : client.pages("ListContacts", {}).next();
      const error = await request.catch((value: unknown) => value);
      expect(error).toBeInstanceOf(HeyError);
      expect(error).toMatchObject({
        code: "usage",
...
it("validates retry ceilings and delay floors before credentials", () => {
  ...
  for (const options of [
    ...
    expect(
      () => new HeyClient({ token: { getToken: token }, ...options }),
    ).toThrow(HeyError);
