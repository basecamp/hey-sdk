...
export interface ClientOptions {
  ...
  maxRetries?: number;
  /** Hold the model's first retry delay to at least this many milliseconds. */
  baseDelayMs?: number;
...
function responseErrorFromText(response: Response, text: string): HeyError {
  let body: unknown;
  if (text) {
    try {
      body = parseJSON(text);
    } catch {
      // The HTTP status classifies an error with a non-JSON body.
    }
  }
...
function retryDelayMs(
  ...
  return (
    Math.max(meta.retry?.baseDelayMs ?? 1000, options.baseDelayMs ?? 0) *
    2 ** attempt
  );
}
async function waitForRetry(ms: number, signal: AbortSignal): Promise<void> {
  ...
  do {
    try {
      await delay(Math.max(0, Math.ceil(deadline - performance.now())), undefined, {
        signal,
      });
    } catch (cause) {
  ...
  async refresh(generation: number) {
    ...
    this.refreshing ??= Promise.resolve()
      ...
      .catch((cause: unknown) => {
        throw new HeyError(
          "auth_required",
          "HEY credentials could not be refreshed",
          0,
          false,
          "",
...
class HttpTransport implements Transport {
  ...
  constructor(
    ...
    if (
      ...
      (!Number.isSafeInteger(options.baseDelayMs) ||
        ...
        options.baseDelayMs > 2_147_483_647)
    )
      throw new HeyError(
        "usage",
        "baseDelayMs must be a nonnegative reliable timer duration",
      );
  }
  url(operation: OperationName, input: RawInput, options: RequestOptions): URL {
    ...
    if (
      options.format === "json" &&
      meta.responseMediaType !== "text/html" &&
      !path.endsWith(".json")
    )
      path += ".json";
  ...
  async request(
    ...
    const retries = Math.min(
      ...
      Math.max(0, (meta.retry?.maxAttempts ?? 1) - 1),
    );
    for (let attempt = 0; ; attempt++) {
      ...
      const headers = new Headers({
        Accept: meta.responseMediaType ?? "application/json",
      ...
      try {
        ...
        signal.throwIfAborted();
        if (meta.safe && attempt < retries) {
          const ms = retryDelayMs(meta, this.options, attempt);
          if (ms <= 2_147_483_647) {
      ...
      if (
        ...
        let ms = retryDelayMs(meta, this.options, attempt);
        if (retryAfter !== null) {
      ...
      if (text && successful && meta.responseMediaType === "text/html")
        data = text;
      else if (text) {
        try {
          data = parseJSON(text);
