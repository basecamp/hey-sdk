...
async function run(tc: Fixture) {
  ...
  try {
    ...
    let client = new HeyClient({
      ...
      baseDelayMs: tc.configOverrides?.baseDelayMs,
    });
...
for (const file of files) {
  ...
  for (const tc of fixtures) {
    ...
    if (exclusion) {
      ...
      if (tc.operation === "GetWorkflowStage") {
        assert.equal(
          (operationMetadata.GetWorkflowStage as { responseMediaType?: string })
            .responseMediaType,
          "text/html",
        );
      } else {
        ...
        assert.ok(
          ...
          "Only the modeled HTML convenience may be excluded",
        );
      }
