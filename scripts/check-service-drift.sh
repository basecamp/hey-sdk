...
# Generic and typed body variants collapse to the same operation.
...
# Browser form operations intentionally call raw WithFormdataBody methods so
# redirect responses remain available to the wrapper.
for f in "$SERVICE_DIR"/*.go; do
  case "$f" in
    *_test.go) continue ;;
  esac
  grep -oE '(\.gen|genClient\(\))\.[A-Za-z0-9_]+' "$f" 2>/dev/null || true
done | sed -E 's/.*\.//' \
  | normalize_operations \
  | sort -u > "$SVC_OPS"
