...
final class HabitsTests: XCTestCase {
    func testAHabitIsWrittenInItsPartsWithTheEmptyOnesLeftOff() async throws {
        ...
        let written = try groupAMember(hey.requests[0].body, "calendar_habit")
        ...
        XCTAssertEqual(
            Set(try groupAMember(hey.requests[1].body, "calendar_habit").keys), ["name"], "fields left empty are kept by HEY, so they are not sent")
    }
}
